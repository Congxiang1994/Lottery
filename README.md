...
**518 首经典英文儿歌列表**：带官方封面，卡片网格展示（序号 + 封面 + 歌名）；
鼠标悬停卡片浮现「**B站 / YouTube**」双按钮，用户自选平台播放（国内不便翻墙也能看 B 站）。
**不下载视频、不内嵌播放器、不接点读**，封面本地托管（国内不被墙）。
前端内置播放进度管理（localStorage，无登录）：已播放打钩 + 完成度进度条、收藏♥、状态/平台筛选/分页、
「回到上次」、最近播放。哔哩哔哩链接由服务器**每晚 00:00 定时任务**刷新（搬运下架自动找回）。
