...
const SPEED_COLOR: Record<number, string> = {
  1: "text-emerald-700 border-emerald-600/25 bg-emerald-500/10",
  2: "text-sky-700 border-sky-600/25 bg-sky-500/10",
  3: "text-amber-700 border-amber-600/25 bg-amber-500/10",
  4: "text-rose-700 border-rose-600/25 bg-rose-500/10",
};
  ...
  return (
    <div className="pt-8">
        ...
        <Reveal className="mt-5">
          <div className="glass rounded-2xl p-5 shadow-card">
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <span className="flex items-center gap-2 text-paper-800">
            ...
            {runStatus.phase === "backtest" ? (
              /* 回测阶段：不确定进度动画 */
              <div className="mt-3 h-2.5 w-full overflow-hidden rounded-full bg-paper-100">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-paper-700">
              {runStatus.phase === "backtest" ? (
                <span className="text-paper-800">预测已写入缓存 · 正在回测全部算法…</span>
              ) : (
                <>
              ...
              {runStatus.lotteries && Object.entries(runStatus.lotteries).map(([lk, st]) => (
                <span key={lk} className="rounded-full border border-paper-200 bg-paper-100 px-2 py-0.5">
                  {lk === "ssq" ? "双色球" : "大乐透"}
                  <span className={st.running ? "text-brand-gold" : "text-emerald-700"}>
                    {" "}{st.done}/{st.total}
                  </span>
                  {st.phase === "backtest" && <span className="text-paper-600"> 回测中</span>}
                  {st.finished_at && !st.running && <span className="text-emerald-700"> ✓</span>}
                </span>
              ))}
            </div>
      ...
      {!runStatus?.running && runStatus && runStatus.finished && (
        <Reveal className="mt-5">
          <div className="flex items-center gap-2 rounded-xl border border-emerald-600/25 bg-emerald-50 px-4 py-2.5 text-xs text-emerald-700">
            <CheckCircle2 size={14} /> 全量运行完成：双色球 + 大乐透 预测（{runStatus.done} 个算法）+ 回测已落库，共识推荐与回测榜已更新
            {runStatus.finished_at && (
              <span className="ml-1 font-mono text-emerald-700/90">
          ...
          <div className="relative">
            ...
            <div className="mt-4 flex flex-wrap items-center">
              {combined ? (
                ...
                <span className="text-xs text-paper-600">暂无数据 — 点击右上角「运行全部算法」或等待每日 0:00 定时跑批</span>
              )}
            </div>
          </div>
        </div>
      ...
      {tab === "plaza" ? (
        <>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1.5 rounded-xl border border-paper-200 bg-paper-100 px-3 py-1.5">
              <Search size={14} className="text-paper-600" />
              <input
                value={q}
                onChange={(e) => setQ(e.target.value)}
                placeholder="搜索算法…"
                className="w-36 bg-transparent text-sm text-paper-900 placeholder-white/30 outline-none"
              />
            </div>
          </div>
            ...
            {filtered.map((a, i) => {
              ...
              return (
                <Reveal key={a.id} delay={Math.min(i, 8) * 40}>
                  <div className={`glass flex h-full flex-col rounded-2xl p-4 shadow-card transition hover:border-paper-300 ${r ? "border-paper-200" : "border-paper-100"}`}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="truncate text-sm font-bold text-paper-900">{a.name}</div>
                        <div className="mt-0.5 flex items-center gap-1.5 text-[10px] text-paper-600">
                      ...
                      <button
                        onClick={() => runOne(a.id)}
                        disabled={run}
                        className="shrink-0 rounded-lg border border-paper-200 bg-paper-100 p-1.5 text-paper-700 transition hover:bg-paper-200 hover:text-paper-900 disabled:opacity-40"
                        title="运行此算法"
                      >
                    ...
                    <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-paper-700">{a.desc}</p>
                    ...
                    {r ? (
                      <div className="mt-3 border-t border-paper-100 pt-3">
                        <div className="flex flex-wrap items-center">
                          ...
                          <span className="mx-0.5 text-sm text-paper-400">+</span>
                          {r.blue.map((n, i) => (
                            <Ball key={`b${i}`} n={n} kind="blue" size={26} delay={100 + i * 30} />
                          ))}
                        </div>
                        <div className="mt-2 flex items-center gap-3 text-[10px] text-paper-600">
                          <span className="flex items-center gap-1"><Gauge size={11} /> {r.elapsed_ms}ms</span>
                          <button onClick={() => toggle(a.id)} className="ml-auto flex items-center gap-1 text-paper-700 hover:text-paper-900">
                      ...
                      <div className="mt-3 flex-1 border-t border-paper-100 pt-3 text-[10px] text-paper-500">
                        {run ? "推演中…" : "点击右上角 ▶ 运行"}
                      </div>
                    )}
                  </div>
          ...
          {filtered.length === 0 && (
            <div className="mt-10 text-center text-sm text-paper-600">没有匹配的算法</div>
          )}
        </>
      ) : (
        <BacktestView
      ...
      {catalog && (
        <p className="mt-8 text-center text-[11px] leading-relaxed text-paper-500">{catalog.disclaimer}</p>
      )}
    </div>
  );
}
    ...
    <div className="mt-2 max-h-56 space-y-1 overflow-y-auto rounded-xl border border-paper-100 bg-paper-100 p-2.5">
      ...
      {entries.map(([k, v]) => (
        <div key={k} className="flex gap-2 text-[10px]">
          <span className="w-28 shrink-0 truncate text-paper-600">{k}</span>
          <span className="break-all text-paper-800">{fmt(v)}</span>
        </div>
      ))}
      {entries.length === 0 && !error && <div className="text-[10px] text-paper-500">无额外信息</div>}
    </div>
  );
}
    ...
    <button
      ...
      className={`flex items-center gap-1 rounded-full border px-3 py-1.5 text-xs transition ${
        active
          ...
          : "border-paper-200 bg-paper-100 text-paper-700 hover:bg-paper-200 hover:text-paper-900"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-sm font-medium transition ${
        active ? "bg-paper-200 text-paper-900" : "text-paper-700 hover:bg-paper-100 hover:text-paper-900"
      }`}
    >
      {icon} {label}
    </button>
  ...
  return (
    <div className="mt-4">
      <div className="glass rounded-2xl p-5 shadow-card">
        ...
        {loading ? (
          <div className="py-10 text-center text-sm text-paper-600">
            {runAllRunning
          ...
          <>
            <div className="mt-4 overflow-x-auto">
              <table className="w-full min-w-[640px] text-left text-xs">
                <thead>
                  <tr className="border-b border-paper-200 text-[10px] uppercase tracking-wider text-paper-600">
                ...
                <tbody>
                  {bt.algos.map((a, i) => (
                    <tr key={a.id} className={`border-b border-paper-100 ${i === 0 ? "bg-brand-gold/5" : ""}`}>
                      <td className="py-2 pr-2 text-paper-600">{i + 1}</td>
                      <td className="py-2 pr-2 font-medium text-paper-900">{a.name}</td>
                      <td className="py-2 pr-2 text-paper-700">{a.category}</td>
                      <td className="py-2 pr-2 text-right text-paper-800">{a.red_avg}</td>
                      <td className="py-2 pr-2 text-right text-paper-800">{a.blue_avg}</td>
                      <td className={`py-2 pr-2 text-right font-semibold ${a.red_lift >= 1 ? "text-emerald-700" : "text-paper-700"}`}>
                        {a.red_lift.toFixed(2)}
                      </td>
                      <td className={`py-2 pr-2 text-right font-semibold ${a.blue_lift >= 1 ? "text-emerald-700" : "text-paper-700"}`}>
                        {a.blue_lift.toFixed(2)}
                      </td>
                      <td className={`py-2 text-right font-bold ${i === 0 ? "text-brand-gold" : "text-paper-900"}`}>
                        {a.score.toFixed(3)}
                      </td>
                    </tr>
                  ))}
            ...
            <p className="mt-3 rounded-lg bg-paper-100 p-2.5 text-[11px] leading-relaxed text-paper-600">{bt.note}</p>
          </>
        ) : null}
      </div>
    </div>
