...
from app.config import LOTTERIES, VERIFY_PASSWORD
from app.services import scraper, stats as stats_svc, predictor
...
def saved_combined(lottery: str):
    ...
    if not res:
        raise HTTPException(status_code=404,
                            detail=f"{lottery} 暂无入库结果，请等待定时任务完成首次跑批")
    import numpy as np
    from app.algorithms.base import normalize, pick_top
...
def run_all_algorithms_all(payload: dict):
    ...
    from app.services.results_store import check_password
